...
export const GET = async (
  ...
  memberMonthlyDuesLogger.debug("GET /api/member-monthly-dues/[societyId]");
  memberMonthlyDuesLogger.info(
    `getting member-monthly-dues for societyId ${params.societyId}`
  );
  ...
  const { status, ...responseData }: Response<GetMemberMonthlyDuesResponse[]> =
    await getMemberMonthlyDuesController(request, monthYear, params.societyId);
